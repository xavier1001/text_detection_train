...
delPaths = []
for p in jpgPath:
    xmlP = os.path.splitext(p)[0] + '.xml'  # os.path.splitext() 分离文件名与扩展名
...
textModel.load_weights('F:/project/chineseocrmodels/models/text.h5')##加载预训练模型权重
...
for layer in textModel.layers[:186]:
    layer.trainable = False

for x in textModel.trainable_weights:
    print(x.name)
print("\n")
...
adam = tf.keras.optimizers.Adam(lr=0.0005, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=1e-6)  # decay=1e-6
...
# 设置检查点
filepath = log_dir + 'weights_epoch{epoch:03d}-loss{loss:.4f}-val_loss{val_loss:.4f}.h5'
checkpoint = ModelCheckpoint(filepath=filepath, monitor='val_loss', verbose=0,
                             save_best_only=False, save_weights_only=True,  mode='auto', period=1)

#textModel.evaluate_generator(generator=testLoad, steps=num_train/batch_size, workers=1)
...
#textModel.evaluate_generator(generator=testLoad, steps=num_train/batch_size, max_q_size=10, workers=1, pickle_safe=False)
textModel.save_weights(log_dir + 'trained_weights.h5')  #只保存了模型的权重
